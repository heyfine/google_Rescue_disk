...
# 捕获 Ctrl+C
trap 'echo -e "\n${RED}[WARN] 检测到中断，正在清理...${NC}";
cleanup; exit 1' SIGINT
...
check_dependencies() {
    local missing=0
    for cmd in zstd pv dd lsblk tar rsync btrfs;
    do
        if ! command -v $cmd &> /dev/null; then missing=1;
        fi
    done
    if [ $missing -eq 1 ];
    then
        ...
        then opkg update && opkg install zstd pv coreutils-dd tar rsync btrfs-progs
        elif command -v apt-get &> /dev/null;
        then apt-get update && apt-get install -y zstd pv tar rsync btrfs-progs;
        fi
    fi
}

stop_services() {
    if command -v systemctl &> /dev/null && systemctl is-active --quiet postgresql;
    then
...
cleanup() {
    ...
    # 清理 Btrfs 临时快照 (如果有)
    # 注意：需要找到挂载点才能删除快照
    if [ -n "$SRC_MOUNT_FOR_SNAP" ] && [ -d "$SRC_MOUNT_FOR_SNAP/$SNAPSHOT_NAME" ];
    then
    ...
    # 卸载临时挂载
    if [ $IS_TEMP_MOUNT -eq 1 ];
    then
        if mount | grep -q "$TEMP_SRC_MOUNT";
        then
...
do_backup() {
    ...
    do
        ...
        if [[ "$SRC_NAME" != /dev/* ]];
        then SRC_DEV="/dev/$SRC_NAME"; else SRC_DEV="$SRC_NAME"; fi
        if [ ! -b "$SRC_DEV" ];
        then echo -e "${RED}[ERROR] 设备不存在${NC}"; sleep 1; continue; fi
        ...
        if [[ "$DEST_PART" != /dev/* ]];
        then DEST_DEV="/dev/$DEST_PART"; else DEST_DEV="$DEST_PART"; fi
        if [ ! -b "$DEST_DEV" ];
        then echo -e "${RED}[ERROR] 分区不存在${NC}"; sleep 1; continue; fi

        # === [新增功能] 检查目标分区是否已格式化 ===
        DEST_FS=$(lsblk -no FSTYPE "$DEST_DEV")
        if [ -z "$DEST_FS" ]; then
            echo -e "\n${RED}[WARN] 检测到目标分区 $DEST_DEV 未格式化！${NC}"
            echo -e "${YELLOW}为了存放备份文件，该分区必须被格式化。${NC}"
            read -p "是否立即将其格式化为 ext4 (数据将被清空)? (y/n): " FORMAT_CONFIRM
            if [ "$FORMAT_CONFIRM" == "y" ]; then
                echo -e "${CYAN}[INFO] 正在格式化...${NC}"
                mkfs.ext4 -F "$DEST_DEV"
                if [ $? -ne 0 ]; then
                     echo -e "${RED}[ERROR] 格式化失败，请检查设备状态。${NC}"
                     sleep 2
                     continue
                fi
                echo -e "${GREEN}[INFO] 格式化完成${NC}"
                sleep 1
            else
                echo -e "${RED}[ERROR] 无法挂载未格式化的分区，请重新选择。${NC}"
                sleep 2
                continue
            fi
        fi
        ...
        then
            ...
            mkdir -p "$MOUNT_POINT"
            mount "$DEST_DEV" "$MOUNT_POINT" ||
            { echo -e "${RED}[ERROR] 挂载失败${NC}"; sleep 2; continue; }
        ...
        case "$MODE_CHOICE" in
            2) B_TYPE="tar";
            FILE_NAME="${DEST_PATH}/backup_${SRC_NAME}_tar_${TIMESTAMP}.tar.${C_EXT}";;
            ...
            *) B_TYPE="dd";
            FILE_NAME="${DEST_PATH}/backup_${SRC_NAME}_dd_${TIMESTAMP}.img.${C_EXT}";;
        esac
        ...
        then
            # 1. DD 模式
            dd if="$SRC_DEV" bs=4M status=none |
            pv -s $(blockdev --getsize64 "$SRC_DEV") | $C_CMD > "$FILE_NAME"
        ...
        then
            ...
            btrfs send "$TARGET_DIR/$SNAPSHOT_NAME" |
            pv | $C_CMD > "$FILE_NAME"
        ...
        cleanup
        if [ -z "$CURRENT_MOUNT" ]; then umount "$MOUNT_POINT";
        fi
        read -p "按回车返回..."
        return
    done
...
do_restore() {
    ...
    do
        ...
        [[ "$PART_NAME" == "0" ]] && return
        if [[ "$PART_NAME" != /dev/* ]];
        then PART_DEV="/dev/$PART_NAME"; else PART_DEV="$PART_NAME"; fi
        if [ ! -b "$PART_DEV" ];
        then echo "[ERROR] 设备不存在"; sleep 1; continue; fi
        ...
        if [ ${#FILES[@]} -eq 0 ];
        then
            echo -e "${RED}[ERROR] 未找到备份文件!${NC}"; umount "$MOUNT_POINT";
            read -p "回车重试..."; continue
        fi

        i=1
        for f in "${FILES[@]}";
        do echo "$i) $(basename "$f")"; ((i++)); done
        ...
        if mount |
        grep "on / type" | grep -q "$DEST_NAME"; then
            echo -e "${RED}[FATAL] 不能在 Live 模式还原到系统盘! 请进入 Rescue 模式。${NC}";
            read -p "回车返回..."; return
        fi
        ...
        then
            ...
            # 解压流
            if [[ "$SEL_FILE" == *.zst ]];
            then D_CMD="zstd -d -c"; elif [[ "$SEL_FILE" == *.gz ]]; then D_CMD="gzip -d -c"; else D_CMD="cat";
            fi
            ...
            then
                ...
                $D_CMD "$SEL_FILE" |
                pv | btrfs receive "$TEMP_SRC_MOUNT"
                umount "$TEMP_SRC_MOUNT"
            else
                # DD 写入
                $D_CMD "$SEL_FILE" |
                pv | dd of="$DEST_DEV" bs=4M status=none
            fi
